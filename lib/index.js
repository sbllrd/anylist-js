...
class AnyList extends EventEmitter {
   ...
   *
   * Re-fetches getLists() afterward rather than constructing the local List
   * ourselves: AnyList's server auto-provisions a default category group
   * for the new list (observed live as an "Untitled Category Set" containing
   * just "Other") that isn't reflected in anything this operation's response
   * tells us. A locally-built stub would have an empty categoryGroups and
   * make List#createCategory wrongly think the list has none at all.
   *
   * allowsMultipleListCategoryGroups is false here (the Rust reference this
   * was copied from always sends true): that flag is what made the list's
   * one default category group show up in the app as a named, user-manageable
   * "Category Set" (seen live as "Untitled Category Set") — unnecessary
   * complexity for a normal list, which just needs headers on
   * (PBListSettings.shouldHideCategories, which defaults to false/shown —
   * nothing to set) and categories in its one default group. NOT yet
   * live-verified with this flag flipped to false — the earlier live tests
   * that confirmed create_category/assignment all ran with it true.
   * If category creation stops finding a default group after this change,
   * this flag is the first thing to suspect.
   ...
   */
	async createList(name) {
		...
		const listFields = {
			...
			creator: this.uid,
			logicalClockTime: 1,
			allowsMultipleListCategoryGroups: false,
			listItemSortOrder: 0,
		...
		if (!list) {
			throw new Error(`List "${cleanName}" was created but could not be found afterward.`);
		}
